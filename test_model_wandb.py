...
def test_all_folders(folders=[],model_name='LSTM_decoder',name='',save_dir=None,window=200,batch_size=1024,return_norm_data=True,norm_type='total',visualize_type='23'):
    ...
    if model_name=='DronePose':
        ...
        input_dim = config.get('input_dim')['value']
        output_dim = config.get('output_dim')['value']
        gru_hidden_size = config.get('gru_hidden_size')['value']
        gru_layers = config.get('gru_layers')['value']
        tcn_channels = config.get('tcn_channels')['value']
        tcn_layers = config.get('tcn_layers')['value']
    ...
    if os.path.exists(checkpoint_path):
        ...
        print("Model file not found")
        exit()    
    batch_size=config.get('batch_size',1024)['value']
