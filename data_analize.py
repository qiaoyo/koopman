    ...
    for folder_idx in stats:
        ...
        for file_key in stats[folder_idx]:
            ...
            for dim in range(len(stats[folder_idx][file_key]['max'])):
                print(f"维度 {dim}:")
                print(f"  最大值: {stats[folder_idx][file_key]['max'][dim]:.4f}")
                print(f"  最小值: {stats[folder_idx][file_key]['min'][dim]:.4f}")
...
def visualize_two_groups(folder1, folder2):
    ...
    # 绘制第一组Motors_CMD数据
    plt.plot(dims_motors, motors_max1, color=colors[0], linestyle='-', marker='o', markersize=6, 
    ...
    # 绘制第二组Motors_CMD数据
    plt.plot(dims_motors, motors_max2, color=colors[1], linestyle='-', marker='o', markersize=6,
